...
```{r structure, echo=FALSE, fig.show='hold', out.width="100%"}
...
```{r plotVAF_example1, message=FALSE, warning=FALSE, fig.cap="Figure 1: VAF distribution of SSNVs"}
strelka_vcf <- system.file("extdata", "strelka2.somatic.snvs.vcf.gz", package = "LACHESIS")
...
```{r nbImport_input, echo=FALSE}
library(knitr)

nb_import_input <- data.frame(
    Column1 = c("*cnv*", "*snv*", "*purity*", "*ploidy*", "*sig.assign*", "*assign.method*", "*ID*", "*sig.file*", "*sig.select*", "*min.p*", "*ref.build*", "*seed*"),
    Column2 = c(
        "output of [readCNV](#output)",
        "output of [readVCF(#output-1)",
        "the tumor cell content",
        "the average copy number in the tumor sample",
        "if TRUE, each variant will be assigned to the most likely mutational signature",
        ...
        "sample name",
        "file path to the SigAssignment output file, typically named `Decomposed_MutationType_Probabilities.txt`",
        "character vector of specific signatures to include in the analysis (e.g., c(´SBS1´, ´SBS5´, ´SBS40´) to focus on clock-like mutational processes)",
        "minimum probability threshold from the SigAssignment output that a variant must meet to be considered as matching a specific signature",
        "reference genome: hg18, hg19 or hg38",
        "User-specified or automatically generated random number, will be documented in the log file."
    )
)
...
```{r plotNB_input, echo=FALSE}
library(knitr)

plotnb_input <- data.frame(
    Column1 = c("*nb*", "*ref.build*", "*min.cn*", "*max.cn*", "*nb.col.abline*", "*nb.col.cn.2*", "*nb.col.cn*", "*nb.col.hist*", "*nb.border*", "*nb.breaks*", "*samp.name*", "*output.file*", "*sig.show*"),
    Column2 = c(
        ...
        "output filename",
        "additional VAF histogram stratified by assigned mutational signatureswill be shown, if set to TRUE."
    )
)
...
```{r plotNB_example1, message=FALSE, warning=FALSE, fig.cap="Figure 2: *top plot* - copy number profile along the genome, *bottom plots* - VAF distribution of SSNVs stratified by copy number and minor/major alleles"}
snvs <- system.file("extdata", "NBE15", "snvs_NBE15_somatic_snvs_conf_8_to_10.vcf", package = "LACHESIS")
s_data <- LACHESIS::readVCF(vcf = snvs, vcf.source = "dkfz")
aceseq_cn <- system.file("extdata", "NBE15", "NBE15_comb_pro_extra2.51_1.txt", package = "LACHESIS")
c_data <- LACHESIS::readCNV(aceseq_cn)
...
```{r plotNB_example2, message=FALSE, warning=FALSE, fig.cap="Figure 2: *top plot* - copy number profile along the genome, *bottom plots* - VAF distribution of SSNVs stratified by copy number and minor/major alleles"}
snvs <- system.file("extdata", "NBE15", "snvs_NBE15_somatic_snvs_conf_8_to_10.vcf", package = "LACHESIS")
s_data <- LACHESIS::readVCF(vcf = snvs, vcf.source = "dkfz")
aceseq_cn <- system.file("extdata", "NBE15", "NBE15_comb_pro_extra2.51_1.txt", package = "LACHESIS")
c_data <- LACHESIS::readCNV(aceseq_cn)
...
```{r clonalmutcount_input, echo=FALSE}
library(knitr)
...
```{r normalizeCounts_input, echo=FALSE}
library(knitr)
...
```{r plotMutDens_input, echo=FALSE}
library(knitr)

plotmrca_input <- data.frame(
    Column1 = c("*mrcaObj*", "*samp.name*", "*min.seg.size*", "*mut.col.zero*", "*mut.col.multi*", "*mut.border*", "*mut.show.density*", "*mut.breaks*", "*mut.xaxis*", "*mut.show.realtime*", "*mut.snv.rate*", "*output.file*"),
    Column2 = c(
        ...
        "interval size [(hist() - breaks)](https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/hist)",
        "optional, user-specified cutoff value for x-axis in evolutionary timeline plot in SNVs/Mb",
        "if TRUE the weeks post-conception or postnatal months will be shown on the evolutionary timeline",
        "rate of accumulated SNVs per day in a diploid genome",
        "output file"
    )
)
...
```{r estimateClonality_input, echo=FALSE}
library(knitr)

estimateClonality_input <- data.frame(
    Column1 = c("*nbObj*", "*mrcaObj*", "*ID*", "*purity*", "*driver.file*", "*ref.build*"),
    Column2 = c(
        "output from [nbImport](#output-2) function",
        "output from [mrca](#output-5) function",
        "the tumor sample name",
        "the tumor cell content",
        "file path to the information about known driver SNVs.",
        "reference genome: hg18, hg19 or hg38"
    )
)

knitr::kable(estimateClonality_input, col.names = c("Input", "Definition"), escape = FALSE)
```
...
```{r plotClonality_input, echo=FALSE}
library(knitr)

estimateClonality_input <- data.frame(
    Column1 = c("*snvClonality*", "*nbObj*", "*sig.assign*", "*output.file*"),
    Column2 = c(
        "output from [estimateClonality](#output-6) function",
        "output from [nbImport](#output-2) function",
        "if TRUE, each variant will be assigned to the most likely mutational signature",
        "output filename"
    )
...
```{r plotLACHESIS_input, echo=FALSE}
library(knitr)
...
plotlachesis_input <- data.frame(
    ...
    Column2 = c(
        "output from [LACHESIS](#output-7) function",
...
```{r plotLACHESIS_example1_2, message=FALSE, warning=FALSE, fig.cap="Figure 4: Cumulative distribution of SSNV densities in ECA (dark green) and MRCA (light green)"}
lachesis <- LACHESIS::LACHESIS(input.files = lachesis_input)
...
```{r plotClinCor_input, echo=FALSE}
library(knitr)
...
```{r plotClinCor_example1, message=FALSE, warning=FALSE, fig.cap="Figure 5: Correlation between age at diagnosis and SNV densities at ECA and MRCA"}
input.files <- system.file("extdata", "Sample_template.txt", package = "LACHESIS")
lachesis <- LACHESIS::LACHESIS(input.files = lachesis_input)
...
```{r plotSurvival_input, echo=FALSE}
plotlachesis_input <- data.frame(
...
```{r plotSurvival_example1, message=FALSE, warning=FALSE, fig.cap="Figure 6: Correlation between Survival and SNV densities at ECA and MRCA"}
input.files <- system.file("extdata", "Sample_template.txt", package = "LACHESIS")
lachesis <- LACHESIS::LACHESIS(input.files = lachesis_input)
...
```{r classifyLACHESIS_input, echo=FALSE}
plotlachesis_input <- data.frame(
    Column1 = c("*lachesis*", "*mrca.cutpoint*", "*output.dir*", "*entity*", "*infer.cutpoint*", "*surv.time*", "*surv.event*"),
    Column2 = c(
        "output from [LACHESIS](#output-6) function",
        "MRCA density value to be used for survival stratification, will be computationally inferred to maximize survival differences if not specified by user",
        "the directory to which the plot will be stored",
        "the tumor entity if classifying according to a pre-defined threshold",
        "the MRCA cutpoint will be inferred from the data if set to TRUE",
        "column name containing survival time",
        "column name containing event"
    )
)
...
```{r classifyLACHESIS_example1, message=FALSE, warning=FALSE, fig.cap="Figure 6: Correlation between Survival and SNV densities at ECA and MRCA"}
input.files <- system.file("extdata", "Sample_template.txt", package = "LACHESIS")
lachesis <- LACHESIS::LACHESIS(input.files = lachesis_input)
