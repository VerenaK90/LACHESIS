...
<!-- Other R Packages with similar aims are [Butte](https://github.com/SunPathLab/Butte), .... -->
...
plotvaf_input <- data.frame(
  ...
  Column2 = c(
    "output from readVCF function: Datatable with chrom, pos, ref, alt, t_ref_count, t_alt_count, t_depth, t_vaf [(vaf)](#output-1)",
    "interval size [(hist() - breaks)](https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/hist)",
    "tumor sample name",
    "showing count per break on the histogram when set to TRUE",
    "showing additional density plot when set to TRUE",
    "color to be used to fill the bars",
    "text angle if *showcounts = TRUE*",
    "saving the plot to this directory and naming it accordingly"
  )
)

kable(plotvaf_input, col.names = c("Output", "Definition"), escape = FALSE)
```
...
nb_import_input <- data.frame(
  ...
  Column2 = c(
    "output from readCNV function: Datatable with Chr, Start, End, A, B and TCN columns [(cnv)](#output)",
    "output from readVCF function: Datatable with chrom, pos, ref, alt, t_ref_count, t_alt_count, t_depth, t_vaf [(snv)](#output-1)",
...
plotmrca_input <- data.frame(
  ...
  Column2 = c(
    ...
    "if TRUE showing the density distribution of mutation densities on single copies in the histogram of mutation densities on multiple copies",
    "interval size (hist() - breaks)[https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/hist]",
    "saving the plot to this directory and naming it accordingly"
  )
)
...
This function calls all of the previously mentioned functions and saves the results to the specified output directory. Input can be specified as vectors or in a tab-delimited **sample-specification file**
...
<!-- lachesis <- LACHESIS::LACHESIS(input.files = input.files) -->
