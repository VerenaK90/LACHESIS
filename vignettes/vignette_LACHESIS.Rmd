...
nb_import_input <- data.frame(
  Column1 = c("*cnv*", "*snv*", "*purity*", "*ploidy*", "*sig.assign*", "*assign.method*", "*ID*", "*sig.file*", "*sig.select*", "*min.p*"),
  Column2 = c(
    ...
    "sample name",
    "file path to the SigAssignment per-sample output file, typically named `Decomposed_MutationType_Probabilities_ID.txt`; For cohort analysis, please merge individual files into a single combined file",
...
[SigProfilerAssignment](https://github.com/AlexandrovLab/SigProfilerAssignment) is a tool developed by Alexandrov lab at UC San Diego. It assigns previously known mutational signatures to sequenced samples. The user must provide an input file in MAF, VCF, or plain text (.txt) format (`input_type` parameter should be set to "vcf", not "matrix"). The tool generates a folder containing per-sample files with decomposed mutation probabilities. For cohort-level analysis, these individual files should be combined into a single file before further processing.
