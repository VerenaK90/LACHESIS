...
plotnb_input <- data.frame(
  ...
  Column2 = c(
    ...
    "the maximum copy number to be included in the analysis",
    "the color code for the abline",
    "the color code if tcn = 2",
    "the color code if other copy numbers",
    "the color code for histograms",
    "the line color",
...
mrca_input <- data.frame(
  ...
  Column2 = c(
    "output from normalizeCounts function: normalized clonal SNV counts stratified [(normObj)](#output-4)",
...
plotmrca_input <- data.frame(
  ...
  Column2 = c(
    "output from mrca function [(mrcaObj)](#output-5)",
    "the tumor sample name",
    "the minimal segment length to be included in the quantification", 
...
plotlachesis_input <- data.frame(
  ...
  Column2 = c(
    "output from LACHESIS function [(lachesis)](#output-6)",
...
plotlachesis_input <- data.frame(
  ...
  Column2 = c(
    "output from LACHESIS function [(lachesis)](#output-6)",
    "value based on SNV_densities_cohort.pdf observation, will be computationally inferred to maximize survival differences if not specified by user",
    ...
    "numeric value to divide survival time by (e.g., 365 for converting days into years, 30 for months)",
...
To ensure valid results, tumor ploidy should be below the max.cn parameter [(readcnv)](#input). LACHESIS default value is max.cn = 4, therefore we recommend excluding samples with ploidy >4. 
...
```
After joining snv and cnv data based on overlap, x variants with empty cnv start column were removed. 
...
```
This warning appears if the datatable generated at nbImport ([(nb)](#output-2)) is empty.
...
```
The datatable generated at normalizeCounts ([(normObj)](#output-4)) only has one row, no reliable estimation of MRCA density is possible. 
...
```
The column *MRCA_time_mean* of LACHESIS wrapper function output [(lachesis)](#output-6) for specified samples is empty, these samples will not be considered for further analysis. 

```
warning("No sample with MRCA density estimate provided. Returning zero.")
```
LACHESIS wrapper function output [(lachesis)](#output-6) is empty, therefore no further analysis will be done. 
...
```
The column named according to parameter surv.time of LACHESIS wrapper function output [(lachesis)](#output-6) for specified samples is empty, these samples will not be considered for further analysis. 

```
warning("No survival events in cohort Returning zero.")
```
The column named according to parameter surv.event of LACHESIS wrapper function output [(lachesis)](#output-6) is empty, there won't be any further analysis. 
