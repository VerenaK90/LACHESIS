...
```
plotMutationDensities(mrcaObj = NULL, samp.name = NULL, min.seg.size = 10^7, mut.col.zero = "#4FB12B", mut.col.multi = "#176A02", mut.border = NULL, mut.show.density = TRUE, mut.breaks = NULL, mut.xaxis = NULL, mut.chr.label = "copy_number", mut.show.realtime = FALSE, mut.snv.rate = 3.2, output.file = NULL)
```
...
plotmrca_input <- data.frame(
  Column1 = c("*mrcaObj*", "*samp.name*", "*min.seg.size*", "*mut.col.zero*", "*mut.col.multi*", "*mut.border*", "*mut.show.density*", "*mut.breaks*", "*mut.xaxis*", "*mut.chr.label*", "*mut.show.realtime*", "*mut.snv.rate*", "*output.file*"),
  Column2 = c(
    ...
    "chromosome label in the evolutionary timeline plot: either `copy_number` to show the segment's copy number state (chr_TCN_A) or `position` to show its location relative to the centromere (chr_p/q/whole)",
