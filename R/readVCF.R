...
#' @param t.sample Sample name for tumor. Must be same as in VCF. Strelka
#' hard codes tumor sample name to "TUMOR"
...
#' Default 30.
#' @param info.af The string encoding the allele frequency field in the FORMAT
#' column. Defaults to `AF`and will be ignored if `vcf.source` != `sentieon`.
#' @param info.dp The string encoding the read depth field in the FORMAT column.
#' Defaults to `DP`and will be ignored if `vcf.source` != `sentieon`.
...
#' )
#' m_data <- readVCF(
#'     vcf = mutect_vcf, vcf.source = "mutect",
#'     filter.value = "."
#' )
...
#' @return a data.table with chrom, pos, ref, alt, t_ref_count, t_alt_count,
#' t_depth, t_vaf
#' @export
...
readVCF <- function(vcf = NULL, ignore.XY = TRUE, vcf.source = "strelka",
    ...
    vcf.sources <- c("strelka", "mutect", "dkfz", "sentieon", "sage")
