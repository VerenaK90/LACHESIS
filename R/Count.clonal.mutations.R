...
#' @param method string; should the clonal mutations be called from vcf files and the output from ACEseq or from the timing information obtained from MutationTimeR? possible values are MutationTimeR and ACEseq
#' @param aceseq.file.name full path to the "comb_pro_extra" file as generated by ACEseq.; only needed if method==ACEseq
#' @param snv.file.name full path to the SNV file as generated by the DKFZ SNV calling pipeline..; only needed if method==ACEseq
#' @param timing.info the bed-file containing the timing information (output from running MutationtimeR); only needed if method==MutationTimeR
#' @param purity.refit should the purity be re-estimated based on the VAF distribution? Defaults to FALSE
#' @param chromosomes vector of chromosomes to be considered. Defaults to 1-22.
#' @param max.CN highest copy number to take into account. Must not exceed 6.
#' @param CpG_TpG logical, should only CpG>TpG mutations be counted?
...
count.clonal.mutations <- function(aceseq.file.name, snv.file.name, timing.info, chromosomes = c(1:22), CpG_TpG=F,
                                   max.CN =4, method="ACEseq", purity., ploidy.){
  ...
  if(method=="ACEseq"){
    ...
    if(CpG_TpG){
      ...
      mutations$vcf <- mutations$vcf[apply(mutations$vcf, 1, function(x){
        sequence_context <- strsplit(x["SEQUENCE_CONTEXT"], split=",")[[1]]

        left <- substr(sequence_context[1], nchar(sequence_context[1]), nchar(sequence_context[1]))
        twobp.left <- paste0(left, x["REF"])

        right <- substr(sequence_context[2], 1, 1)
        twobp.right <- paste0(x["REF"], right)

        if((twobp.right=="CG" & x["ALT"]=="T" )| (twobp.left=="CG" & x["ALT"]=="A" )){
          return(T)
        }else{
          return(F)
        }
    ...
    clonal.mutation.matrix.this.tumor <- matrix(0, nrow=length(copy.number.indicator), ncol=length(chromosomes),
                                                dimnames = list(apply(rbind(copy.number.indicator, B.allele.indicator), 2, function(x){
                                                  state.name <- state.df[state.df$CN==x[1],]$name
                                                  state.b <- state.b[state.b$B==x[2],]$name
                                                  paste(state.name, state.b, sep="_")
    ...
    segment.length.matrix.this.tumor <- clonal.mutation.matrix.this.tumor

    timing.info$CN <- timing.info$minor_cn + timing.info$major_cn
    ...
    for(j in 1:22){
      ...
      for(k in unique(copy.number.indicator)){
        ...
        for(l in 0:k){
          ...
          tmp <- timing.info[timing.info$seqnames==j & timing.info$CN==k &
                               timing.info$minor_cn==l,,drop=F]

          if(nrow(tmp)==0){next}
          ## require at least 10^7bps with this copy number state
          if(sum(tmp$ends - tmp$starts)<=10^7){next}
          ## quantify early and late SNVs
          late <- sum(tmp$n.single.copy.clonals)
          early <- sum(tmp$n.early.clonals)

          ## minor copy number
          clonal.mutation.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==1),j] <- clonal.mutation.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==1),j] + sum(late)
          segment.length.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==1),j]  <- segment.length.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==1),j] + sum(tmp$ends - tmp$starts)

          ## major copy number
          if(k-l!=1){
            clonal.mutation.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==k-l),j] <- clonal.mutation.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==k-l),j] + sum(early)
            segment.length.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==k-l),j]  <- segment.length.matrix.this.tumor[which(copy.number.indicator==k & B.allele.indicator==k-l),j] + sum(tmp$ends - tmp$starts)
    ...
    return(list(clonal.mutation.matrix.this.tumor = clonal.mutation.matrix.this.tumor,
                segment.length.matrix.this.tumor = segment.length.matrix.this.tumor,
                copy.number.indicator = copy.number.indicator, B.allele.indicator = B.allele.indicator))
