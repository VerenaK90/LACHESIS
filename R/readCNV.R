...
readCNV <- function(cn.info = NULL, chr.col = NULL, start.col = NULL,
  ...
  format.cnv <- .format_cnv_data(cn.info = cn.info, chr.col = chr.col,
  ...
  if (grepl("chr", cn.info[[chr.col]][1])){
    ...
    cn.info[[chr.col]] <- gsub(pattern = "chr", replacement = "",
                               x = cn.info[[chr.col]])
  }
...
.format_cnv_data <- function(cn.info = NULL, chr.col = NULL, start.col = NULL,
  ...
  if(is.null(chr.col) || is.na(chr.col)){
    chr.col <- colnames(cn.info)[grepl("chr", colnames(cn.info),
                                       ignore.case = TRUE)] # try to match with standard nomenclature
    ...
    chr.col <- match.arg(arg = chr.col, choices = colnames(cn.info),
                         several.ok = FALSE)
  ...
  if(is.null(start.col) || is.na(start.col)){
    ...
    start.col <- match.arg(arg = start.col, choices = colnames(cn.info),
                           several.ok = FALSE)
  ...
  if(is.null(end.col) || is.na(end.col)){
    ...
    end.col <- match.arg(arg = end.col, choices = colnames(cn.info),
                         several.ok = FALSE)
  ...
  if(is.null(A.col) || is.na(A.col)){
    ...
    A.col <- match.arg(arg = A.col, choices = colnames(cn.info),
                       several.ok = FALSE)
  ...
  if(is.null(B.col) || is.na(B.col)){
    ...
    B.col <- match.arg(arg = B.col, choices = colnames(cn.info),
                       several.ok = FALSE)
  ...
  if(is.null(tcn.col) || is.na(tcn.col)){
    if(!is.null(A.col) & !is.null(B.col)){
      ...
      tcn.col <- colnames(cn.info)[grepl("\\btcn\\b", colnames(cn.info),
                                     ...
                                     grepl("\\bcopy number\\b",
                                           colnames(cn.info), ignore.case = TRUE)] # try to match with standard nomenclature
      if(length(tcn.col)==0){
    ...
    tcn.col <- match.arg(arg = tcn.col, choices = colnames(cn.info),
                         several.ok = FALSE)
  ...
  return(list(cn.info = cn.info, chr.col = chr.col, start.col = start.col,
