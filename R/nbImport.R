...
#' plotNB(nb, sig.show = TRUE)
#' estimateClonality(nb, ID = "NBE15", purity = 1)
#'
#' @export

estimateClonality <- function(nb = NULL, ID = NULL, purity = NULL) {
  ...
  snvClonality <- data.table::copy(nb)
  View(snvClonality)
  ...
  snvClonality[, Clonality := {
    ...
    if (is.na(CN) || is.na(VAF) || CN == 0 || depth == 0) {
      ...
      if (VAF < qbinom(p = 0.05, size = depth, prob = expectedVAFs[1]) / depth) {
        "SC"  # Subclonal

      } else if (CN > 2 && VAF < qbinom(p = 0.05, size = depth, prob = expectedVAFs[2]) / depth) {
...
#' nb <- nbImport(cnv = c_data, snv = s_data, purity = 1, ploidy = 2.51, sig.assign = TRUE, ID = "NBE15", sig.file = sig.filepath)
#' plotNB(nb, sig.show = TRUE)
#' snvClonality <- estimateClonality(nb, ID = "NBE15", purity = 1)
