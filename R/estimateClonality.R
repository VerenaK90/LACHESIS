...
estimateClonality <- function(nbObj = NULL, mrcaObj = NULL, ID = NULL,
      ...
      A <- A_time <- B <- B_time <- Clonality <- Sample <- Signature <- TCN <- alt <-
      chrom <- cn_end <- cn_start <- known_driver_gene <- p_c <- p_ec <- p_lc <-
      p_sc <- ref <- snv_start <- t_alt_count <- t_depth <- t_vaf <- . <- NULL
