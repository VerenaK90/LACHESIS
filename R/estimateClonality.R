...
estimateClonality <- function(nbObj = NULL, mrcaObj = NULL, ID = NULL, purity = NULL) {

  if (is.null(nbObj) || is.null(mrca)) {
  ...
  nbObj <- merge(nbObj, mrca[, .(chrom, TCN, A, B, p_sc, p_lc, p_ec, p_c, A_time, B_time)],
...
#' norm_muts <- normalizeCounts(cl_muts)
#' mrca <- MRCA(norm_muts)
#' estimateClonality(nbObj = nb, mrca = mrca, ID = "NBE15", purity = 1)
#' plotClonality(snvClonality, nb = nb, sig.assign = TRUE)
#'
#' @import ggplot2
#' @export

plotClonality <- function(snvClonality = snvClonality, nb = NULL, sig.assign = FALSE, output.file = NULL, ...) {
