...
estimateClonality <- function(nbObj = NULL, mrcaObj = NULL, ID = NULL,
                              purity = NULL, driver.file = NULL, ref.build = "hg19") {

  if (is.null(nbObj) || is.null(mrcaObj)) {
    stop("Missing input. Please provide the output generated by nbImport and
  ...
  snvClonality <- merge(nbObj, mrcaObj[, .(chrom, TCN, A, B, p_sc, p_lc, p_ec,
                                           p_c, A_time, B_time)],
  ...
  snvClonality[, Clonality := {
    ...
    if (is.na(CN) || is.na(VAF) || CN == 0 || depth == 0) {
      ...
      likelihoods <- c(SC = lik_SC, Postcnv = lik_postcnv,
...
plotClonality <- function(snvClonality = snvClonality, nbObj = NULL,
  ...
  chrom_levels <- unique(snvClonality$chrom_AB[order(as.numeric(sub("chr", "",
                                                                    snvClonality$chrom)))])
  snvClonality <- snvClonality[Clonality %in% c("Precnv", "Postcnv", "SC", "C") &
                                 !is.na(chrom_AB)]
