...
#' \item{`n_mut_total`}{Normalized number of mutations per single copy of the segment}
...
MRCA <- function(normObj = NULL, min.seg.size = 10^7, fp.mean = 0, fp.sd = 0, excl.chr = NULL){
  ...
  tot.muts.mrca <- workObj[,sum(n_mut_total)]
  ...
  bootstrapped.mrca.time <- sapply(1:1000, function(x){
    ...
    res <- workObj[res, sum(n_mut_total)/sum(Seglength)]*10^6
  ...
  mut.counts.eca <- c(workObj[A>1 & density_A_mean <= mutation.time.mrca & p_adj_A_to_mrca < 0.01, n_mut_A], workObj[B>1 & A!=B & density_B_mean <= mutation.time.mrca & p_adj_B_to_mrca < 0.01, n_mut_B])
  seg.length.eca <- c(workObj[A>1 & density_A_mean <= mutation.time.mrca & p_adj_A_to_mrca < 0.01, Seglength], workObj[B>1 & A!=B & density_B_mean <= mutation.time.mrca & p_adj_B_to_mrca < 0.01, Seglength])
  ...
  workObj$A_time <- apply(workObj[,c("A", "p_adj_A_to_mrca", "p_adj_A_to_eca")], 1, function(x){
    if(x[1] <= 1){return(NA)}
  ...
  workObj$B_time <- apply(workObj[,c("A", "A_time", "B", "p_adj_B_to_mrca", "p_adj_B_to_eca")], 1, function(x){
    if(x[3] <= 1){return(NA)}
    if(x[1]==x[3]){
      return(x[2])
