...
plotMutationDensities <- function(mrcaObj = NULL, samp.name = NULL,
  ...
  Seglength <- . <- A <- B <- variable <- value <- lines <- density <-
    ...
    p_total_to_mrca <- p_A_to_mrca <- p_B_to_mrca <-
    p_adj_total_to_mrca <- p_adj_A_to_mrca <- p_adj_B_to_mrca <-
    MRCA_qual <- p_A_to_eca <- p_B_to_eca <- p_adj_A_to_eca <-
    p_adj_B_to_eca <- A_time <- B_time <- NULL
  ...
  to.plot <- data.table::melt(mrcaObj,
                              ...
                              measure.vars = c("density_total_mean",
                                               "density_A_mean",
                                               "density_B_mean"))
  ...
  if(nrow(to.plot[(variable == "density_A_mean" & A > 1) |
    ...
    plot(NA, NA, xlim = c(0, 1), ylim = c(0, 1), xlab = NA, ylab = NA,
  ...
  if(!is.null(mut.xaxis) && !is.na(mut.xaxis)) {
    ...
    x.max <- max(c(mrcaObj$density_total_upper, mrcaObj$density_A_upper,
                   mrcaObj$density_B_upper), na.rm = TRUE)*1.3
  }
  y.min <- 0
  ...
  if(mut.show.realtime){
    ...
    axis(side = 3, at = c(x.min, snvs_per_mb, x.max), labels = c("", realtime_labels, ""),
         cex.axis = 0.7)
    segments(x0 = x.min, y0 = par("usr")[4], x1 = x.max, y1 = par("usr")[4], xpd = NA)
    mtext("Estimated time (weeks post conception and months postnatal)", side = 3,
          line = 2, cex = 0.7)
  ...
  # ECA:
  polygon(c(attr(mrcaObj, "ECA_time_lower"), rep(attr(mrcaObj, "ECA_time_upper"), 2),
            attr(mrcaObj, "ECA_time_lower")), c(rep(y.min, 2), rep(y.max, 2)),
          col = mut.col.multi, border = NA)
  abline(v = attr(mrcaObj, "ECA_time_mean"), lty = 2)

  #MRCA:
  polygon(c(attr(mrcaObj, "MRCA_time_lower"), rep(attr(mrcaObj, "MRCA_time_upper"), 2),
            attr(mrcaObj, "MRCA_time_lower")), c(rep(y.min, 2), rep(y.max, 2)),
  ...
  chr_label <- paste0("chr", mrcaObj$chrom, mrcaObj$chr_region, " (", mrcaObj$A,
                      ":", mrcaObj$B, ")")
  ...
  if(nrow(mrcaObj[A > 1,])>0){
    ...
    arrows(x0 = mrcaObj[A > 1,density_A_lower], y0 = seq_len(mrcaObj[,sum(A > 1)]),
           x1=mrcaObj[A > 1,density_A_upper], y1 = seq_len(mrcaObj[,sum(A > 1)]),
           code = 3, angle = 90, length = 0, col = chrom_colors[mrcaObj[A > 1, chrom]],
           lwd = 1)
    text(x = mrcaObj[A > 1, density_A_upper], y = seq_len(mrcaObj[, sum(A > 1)]),
         labels = chr_label[mrcaObj$A > 1], cex = 0.6, pos = 4)
    legend("topright",box.lwd = 0, pch = signs[names(signs) %in% mrcaObj$A_time |
  ...
  if(nrow(mrcaObj[B > 1 & B != A,])>0){
    ...
    arrows(x0 = mrcaObj[B > 1 & B != A,density_B_lower],
    ...
    text(x = mrcaObj[B > 1 & B != A, density_B_upper],
...
.getCentromeres <- function(build = "hg19"){
  if(build == 'hg19') {
    centromeres <- c(123400000, 93900000, 91000000, 50000000, 48800000, 60000000,
    ...
    centromeres <- c(121000000, 93300000, 91000000, 50000000, 48800000, 60000000,
                     58000000, 45000000, 50000000, 39500000, 53000000, 35500000,
                     17500000, 18000000, 36500000, 36000000, 38000000, 18500000,
                     27500000, 29000000, 14000000, 12000000, 61000000, 17500000)
  }else if (build == 'hg38') {
    centromeres <- c(123400000, 93300000, 91000000, 50000000, 48800000, 60000000,
                     58000000, 45000000, 50000000, 39500000, 53000000, 35500000,
                     17500000, 18000000, 36500000, 36000000, 38000000, 18500000,
                     27500000, 29000000, 14000000, 12000000, 61000000, 17500000)
  }else{
